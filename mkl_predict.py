...
symbolIdx = 6  # pick one of the symbols
...
if __name__ == '__main__':
    ...
    cleanKernelsFileName = "/".join((MKLSymbolKernelsPath, "cleanKernelsList.pkl"))
    loadPickleKernels = pkl.load(open(cleanKernelsFileName, "rb"))

    for kernelKey in loadPickleKernels.keys():

        Xtr, ytr = featureCreation(idxKey=kernelKey,
        ...
        if Xtr.shape[0] == ytr.shape[0]:
            if np.isfinite(Xtr).all() == True and np.isnan(Xtr).all() == False:
                kernelHPK = [HPK_kernel(Xtr, degree=d) for d in range(1, 5)]
                kernelRBF = [RBF(Xtr, gamma=gamma) for gamma in [1., 10, 100.]]
                print('training EasyMKL...for polynomials and RBF', end='')
                kernelFileName = "/".join(
                    (MKLSymbolKernelsPath, "".join((kernelKey[0], '_', kernelKey[1], "_Kernels.pkl"))))
                compKernels[kernelKey] = [kernelHPK, kernelRBF]
                pkl.dump(compKernels, open(kernelFileName), "wb")
            else:
                print('Shapes dont match.')
            continue
    pkl.dump(compKernels, open("/".join((MKLSymbolKernelsPath, "completeKernelsList.pkl")), "wb"))
